...
/**
 * 运行时织入检查：伪装硬度是否与黑曜石一致。单测只覆盖 ID 匹配；
 * 此处命中 {@code BlockState.getDestroyProgress}、爆炸抗性与
 * {@code canEntityDestroy}，确认 mixin 生效后行为一致。
 *
 * <p>仅 Create 在场时注册（见 {@code DimBlend#onRegisterGameTests} 的 ModList 守卫），
 * 模板结构见 {@code data/dimblend_experience/structure/copycat_obsidian.nbt}。
 */
...
public final class CopycatObsidianHardnessGameTests {
    ...
    @GameTest(template = "copycat_obsidian", templateNamespace = "dimblend_experience", timeoutTicks = 200)
