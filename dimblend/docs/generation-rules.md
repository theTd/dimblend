...
| 区域范围 | 随机池 |
|---|---|
| 21–31 | 以上纬度（0–20 已出现的类型：地表 / 地下 / 地狱 / 天域 / 暮色 / 星光 / 深暗；不含末地、深渊） |
| 34 以上 | 全纬度随机：全部纬度（含末地、深渊） |

### 1.3 随机约束

- **随机区段**的非地表 delegate 不得与相邻区相同（含 31|32、34|33，负侧对称）。固定 0–20 仍允许连续同类型（如 8/9 地表、5/6 地下、11/12 地狱）。
- 随机区地表不得孤立：连续长度至少 2，允许更长。可以接在固定地表 |20| 上（20|21 同为地表合法），也可以跨远窗边界（49|50 及负侧对称）。31 与 −31 不能新开一段，因为向外是固定末地，接不上；30|31 作为一段的结尾合法。
- 地表具有 **2 倍随机权重**。权重只作用于新开一段、或在已经合法的段上再加一格的那一次抽签；用来凑满两格的下一格直接沿用同一地表 delegate，不再抽签。
- 每个随机窗口内，当前池里的**每一种地形**（按 delegate，暮色/星光/天域等分开计）至少出现一次：21–31（池为 1.2 的 7 种以上纬度），以及 34 起每 16 区一段（池为全纬度）。正负两侧窗口独立保底。
- 保底按代价强制，不回溯。未出现的非地表每个占 1 格；未出现的地表在能接上已有地表时占 1 格，否则占 2 格（窗内只剩 1 格且向外仍是随机区时占 1 格，另一格跨到下一窗）。剩余格数 ≤ 总代价时只在未出现种类里抽。窗口比最低代价宽（中窗余 3，远窗余 6），不会抽到死结。
...
- [x] 地形相对原维度下移 64（`dimblend:y_shifted` 包 `voidscape:void`，`y_offset: -64`；走廊仍在 Y=64，相对岛面抬高 64。密度/群系走与暮色相同的 `YShiftedDensity`，保留 Voidscape 自己的 chunk generator。**Y 平移必须落在 `minecraft:interpolated` 标记之下**：`YShiftedNoiseSettings.shifted` 只给不含 interpolated 子树的节点包 `YShiftedDensity`——包装在插值标记之上时，`NoiseChunk` 的 `NoiseInterpolator` 收到的是移位委托 context（≠ NoiseChunk），走 `noiseFiller.compute` 原始路径，角点三线性插值被整体旁路，岛形变成逐块原始噪声、与原维度完全不同。同种子同区 band vs `voidscape:void` 逐格对比（同一 forceload 区块带 z∈[18,27]（block Z 288–447）内抽样 9 对 chunk：x=2563/2568/2573 × z=19/20/23，结构起点均为 0）：修复前每 chunk 失配 1.0-2.3 万格；修复后岛形逐格对齐，失配降至 145-4738 格且全部是表面材质/装饰patch 级（bedrock↔null_black/holystone 互换），无轮廓差异。幂等判定同步改为全树扫描（`existingShiftOffset`），旧世界按叶包裹形态落盘后重载仍命中早退）
- [x] 残余差异声明（非 y-shift 缺陷）：同种子重复生成 `voidscape:void` 自身，群系层边界（layerBottomDownwardsStart/sliceBottom/sliceTop/layerTopUpwardsStart 的 ±2 合并窗）附近每 chunk 仍有 0-25 个群系 quart 抖动——`LayeredBiomeProvider` 的 `layerMergeRandom` 共享实例与 `LazyArea` 缓存非线程安全，并发填充 chunk 时两次生成结果即不同（对照实验：void 维度 run1 vs run2 biome quart 失配 0-25/块失配 0-37）。深渊带 vs 真维度对比里看到的表面材质差异（表面规则经 `biomeManager::getBiome` 实时重查群系源，同样命中该竞态）与矿簇/植被 patch 差异同源；band 侧另有装饰行下探到世界底的流体池与顶部 8 条额外装饰行带来的少量多余 patch（见反尖塔条目同族的绝对 Y 约定）
